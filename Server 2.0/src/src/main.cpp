...
main_funcs main_data = {&logging, &timeout_client, &app, &ipAddr, port, &space, std::make_shared<int>(nfds), &PAGE_PATH,
                        NULL, false, {}, &sqller, std::make_shared<sock_handle>(sock), &client_map, &module } ;
void close_epolls() {
...
int main() {
    ...
    module.pwd = path;
    read_cfg();
    if (main_data.config_Data["SERVER"]["CONFIG"].contains("MODS")) {
        if (main_data.config_Data["SERVER"]["CONFIG"]["MODS"]["ENABLE"].get<bool>()) {
            module.init_units(reinterpret_cast<modules::main_funcs *>(&main_data));
        }
    }
    ...
    try {
        ...
        if(app.is_api(info.request_info.path)){
            try {
                ...
                app.routes[info.request_info.path](&compressed_data, &rep, info, app.access, static_cast< void* >(&main_data) );
